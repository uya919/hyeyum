...
import type { ClassRecord, Class } from '../types';

interface ProgressRecordModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (record: ClassRecord) => void;
  recordToEdit: ClassRecord | null;
  classInfo: Class; 
...
const ProgressRecordModal: React.FC<ProgressRecordModalProps> = ({ 
    isOpen, 
    onClose, 
    onSave, 
    recordToEdit, 
  ...
  useEffect(() => {
    if (isOpen) {
      setError('');
      if (recordToEdit) { // Editing
        setDate(recordToEdit.date);

        const isProgTbInList = classInfo.progressTextbooks.includes(recordToEdit.progressTextbook);
        setProgressTextbook(recordToEdit.progressTextbook);
        setIsManualProgressTextbook(!isProgTbInList && !!recordToEdit.progressTextbook);

        const pRange = recordToEdit.progressRange || '';
        const pMatch = pRange.match(/(\d+)-?(\d*)/i);
        setProgressStartPage(pMatch ? pMatch[1] : '');
        setProgressEndPage(pMatch ? (pMatch[2] || pMatch[1]) : '');

        const isHwTbInList = classInfo.homeworkTextbooks.includes(recordToEdit.homeworkTextbook);
        setHomeworkTextbook(recordToEdit.homeworkTextbook || '');
        setIsManualHomeworkTextbook(!isHwTbInList && !!recordToEdit.homeworkTextbook);

        const hRange = recordToEdit.homeworkRange || '';
        ...
        setIsCompleted(recordToEdit.isCompleted);
      } else { // Adding new
        const today = new Date().toISOString().split('T')[0];
        setDate(today);

        const lastRecord = [...classInfo.records].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
        const defaultProgTb = lastRecord ? lastRecord.progressTextbook : '';
        const defaultHwTb = lastRecord ? lastRecord.homeworkTextbook : '';
        
        setProgressTextbook(defaultProgTb);
        setIsManualProgressTextbook(!classInfo.progressTextbooks.includes(defaultProgTb) && !!defaultProgTb);
        
        setHomeworkTextbook(defaultHwTb);
        setIsManualHomeworkTextbook(!classInfo.homeworkTextbooks.includes(defaultHwTb) && !!defaultHwTb);
        
        setProgressStartPage('');
        setProgressEndPage('');
        setHomeworkStartPage('');
        setHomeworkEndPage('');
        setMemo('');
        setIsCompleted(false);
      }
    }
  }, [isOpen, recordToEdit, classInfo]);
  ...
  }, [date, progressTextbook, progressStartPage, progressEndPage, homeworkTextbook, homeworkStartPage, homeworkEndPage, memo, isCompleted, onSave]);
        ...
        >
          ...
          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg mb-6 text-center">
              <div className='flex items-center justify-center space-x-2'>
                  ...
                  <h3 className="text-md font-bold text-blue-800 dark:text-blue-200">{classInfo.name}</h3>
              </div>
          </div>

          <div className="space-y-4 mb-6">
            <div>
              <label htmlFor="recordDate" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">날짜 <span className='text-red-500'>*</span></label>
              <input
                type="date"
                id="recordDate"
                value={date}
                ...
                disabled={isEditing}
                className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="수업 중 특이사항이나 다음 시간 준비사항을 적어주세요."
                className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
             <div className="flex items-center">
              ...
              />
              <label htmlFor="isCompleted" className="ml-2 block text-sm text-slate-900 dark:text-slate-200">
                수업 완료
              </label>
            </div>
